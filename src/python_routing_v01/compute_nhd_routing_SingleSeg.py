...
def compute_network(
    ...
    logger.info("Compute network with flowveldepth_connect %s \n, terminal_segment %s \n, supernetwork_parameters %s \n, waterbody_parameters %s \n, \
waterbody %s \n, nts %s \n, dt %s \n, qts_subdivisions %s \n, verbose %s \n, debuglevel %s \n, csv_output %s \n, nc_output_folder %s \n, \ assume_short_ts %s", 
    flowveldepth_connect,
    terminal_segment,
    supernetwork_parameters,
...
def compute_mc_reach_up2down(
    ...
    while True:
        ...
        if current_segment == reach["reach_tail"]:
            if debuglevel <= -2:
                logger.warning(f"{current_segment} (tail)")
            break
        if debuglevel <= -3:
            logger.warning(f"{current_segment} --> {next_segment}\n")
...
def compute_level_pool_reach_up2down(
    ...
    if debuglevel <= -2:
        logger.warning(f"executing reservoir computation on waterbody: {waterbody}")
...
def writeNC(
    ...
    ncfile.close()
    if debuglevel <= -1:
        logger.warning(f"{filename} closed!")
...
def main():
    ...
    for nsq in range(max_network_seqorder, -1, -1):
        ...
        if parallel_compute:
            ...
            if debuglevel <= -2:
                logger.warning(f"reaches to be routed include:")
